...
format:
  ...
  pdf: default
abstract: |
  13C Metabolic flux analysis @wiechert13CMetabolicFlux2001a aims to infer the
...
is known as the "forward problem". [REFERENCE] shows how, assuming that the
network is in a metabolic and isotopic steady state, so that neither the
concentrations of the compounds nor the distributions of isotopomers are
changing, it is possible to calculate the isotopomer distribution for each
compound given a known flux; in this way it is possible to calculate the
labelling pattern $r(v)$ corresponding to any flux assignment $v$.
...
The inverse problem of inferring steady state fluxes from measured isotopomer
distributions can be solved using a statistical model that links these
measurements with latent parameters representing flux configurations. In
general, such a model specifies the probability $p(r_{obs}\mid r(v))$ of
observing labelling pattern $r_{obs}$ given a true flux assignment $v$ and true
labelling pattern $r(v)$. For example, assuming a linear model, or equivalently
optimising $v$ by least squares, yields the following relationship:

$$
\begin{equation}
r_obs \sim N(r(v), \Sigma)
\end{equation}
$$
...
isotopomer distribution vectors are compositional. We therefore considered it
...
explicitly as in [@theorellBeCertainUncertainty2017b, Eq. 3] or more commonly
